...
class DQNPlayer(BasePokerPlayer):
    def __init__(
        ...
        # note: if we need to use analyze.py, we need to undenote the below two lines
        with open('./card_estimation.pkl', 'rb') as f:
            self.hole_card_est = pickle.load(f)
        # note: if we need to use start_game.py, we need to undenote the below two lines
        ...
        if is_restore:
            # note: if we need to use analyze.py, we need to undenote the below one lines
            self.model.load_state_dict(torch.load('./model_base.pth'))
            # note: if we need to use start_game.py, we need to undenote the below one lines
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        win_rate = estimate_hole_card_win_rate(10000, len(round_state['seats']), hole_card, round_state['community_card'])
